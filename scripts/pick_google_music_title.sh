...
song=$(echo | yeganesh -p songs -- -nb "#2d2d2d" -sb "#37526e" -i -l 10 -b -fn "Noto Sans CJK KR-12")
echo $song
...
station_url="http://localhost:9999/get_new_station_by_search?type=song"
echo $station_url

song_urls=$(curl -G --data-urlencode title="$song" -s $station_url | grep -Po 'http.*')

...
config_file=~/scripts/weather_rc
...
then
	...
	output="$background$text $greeting_text $city $delimiter\n$data $temperature $scale $icon$dashes$text $wind_text $delimiter$data $wind $speed_unit $direction $dashes$text $humidity_text $delimiter$data $humidity % $dashes$text $pressure_text $delimiter$data $pressure $pressure_unit"
